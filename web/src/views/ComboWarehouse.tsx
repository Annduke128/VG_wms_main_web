...
function CatalogTab() {
	...
	const [bomSemi, setBomSemi] = useState<
		{ ma_component: string; so_luong: number }[]
	>([]);
	const [bomAccessory, setBomAccessory] = useState<
		{ ma_component: string; so_luong: number }[]
	>([]);
	...
	return (
		<div>
					...
					>
						...
						<div>
							...
							<input
								...
								onChange={(e) =>
									setForm({ ...form, ten_combo: e.target.value })
								}
					...
					<div style={{ marginBottom: 12 }}>
						...
						>
							<label style={{ ...labelStyle, margin: 0 }}>
								BOM — Bán thành phẩm (kho chính)
							</label>
							<button
								style={{ ...btnOutline, padding: "4px 10px", fontSize: 12 }}
								onClick={() =>
									setBomSemi([...bomSemi, { ma_component: "", so_luong: 1 }])
								}
							>
								+ Thêm
							</button>
						...
						{bomAccessory.map((item, i) => (
							<div key={i} style={{ display: "flex", gap: 8, marginBottom: 4 }}>
								...
								<button
									...
									onClick={() =>
										setBomAccessory(bomAccessory.filter((_, j) => j !== i))
									}
								>
									✕
								</button>
				...
				<div style={card}>
					...
					>
						...
						<div style={{ display: "flex", gap: 8 }}>
							...
							<button
								style={btnDanger}
								onClick={() => handleDelete(selected.ma_combo)}
							>
								Xóa
							</button>
							<button
			...
			{loading ? (
				...
				<p style={{ color: "#888" }}>
					Chưa có combo nào. Nhấn "Thêm combo" để bắt đầu.
				</p>
			) : (
				<table
					style={{
				...
				>
					...
					<tbody>
						{masters.map((m) => (
							...
							>
								<td style={{ ...tdStyle, fontWeight: 600, color: "#6b7efa" }}>
									{m.ma_combo}
								</td>
...
function ComboActionTab({
	actionType,
}: {
	actionType: "create" | "export" | "return";
}) {
	const [masters, setMasters] = useState<ComboMaster[]>([]);
	...
	const execute = async (action: "create" | "cancel" | "out" | "return") => {
		...
		setResult(null);
		try {
			const body = { ma_combo: maCombo, so_luong: Number(soLuong), note };
			...
			>
				<div>
					...
					<select
						style={inputStyle}
						value={maCombo}
						onChange={(e) => setMaCombo(e.target.value)}
					>
	...
	const load = useCallback(async () => {
		...
		try {
			const [inv, txn] = await Promise.all([
				api.comboInventory(),
				api.comboTransactions(txPage, 20),
			]);
...
function AccessoriesTab() {
	...
	const handleStockIn = async () => {
		if (
			!stockForm.ma_phu_kien ||
			!stockForm.so_luong ||
			Number(stockForm.so_luong) <= 0
		) {
					...
					>
						<div>
							...
							<input
								...
								onChange={(e) =>
									setAddForm({ ...addForm, ma_phu_kien: e.target.value })
								}
						...
						<div>
							...
							<input
								...
								onChange={(e) =>
									setAddForm({ ...addForm, ten_phu_kien: e.target.value })
								}
								placeholder="Tên hiển thị"
							/>
						</div>
						<div>
							...
							<input
								...
								onChange={(e) =>
									setAddForm({ ...addForm, don_vi_tinh: e.target.value })
								}
				...
				>
					<div>
						...
						<select
							...
							onChange={(e) =>
								setStockForm({ ...stockForm, ma_phu_kien: e.target.value })
							}
						>
					...
					<div>
						...
						<input
							...
							onChange={(e) =>
								setStockForm({ ...stockForm, so_luong: e.target.value })
							}
						/>
					</div>
					<div>
						...
						<tbody>
							{inventory.map((inv) => (
								...
								>
									...
									<td style={tdStyle}>
										{new Date(inv.updated_at).toLocaleDateString("vi-VN")}
									</td>
								</tr>
							))}
						</tbody>
				...
				{TABS.map((t) => (
					<button
						...
						style={{
							...
							cursor: "pointer",
							borderBottom:
								tab === t.key ? "2px solid #6b7efa" : "2px solid transparent",
							marginBottom: -2,
