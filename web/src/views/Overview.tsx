  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      const [s, c, a, zs, ra] = await Promise.all([
        api.dashboardSummary(activeWarehouseId) as Promise<DashboardSummary>,
        api.dashboardCharts(activeWarehouseId, 4) as Promise<DashboardCharts>,
        api.inventoryAlerts(activeWarehouseId) as Promise<AlertItem[]>,
        api.dashboardZeroSales(activeWarehouseId) as Promise<ZeroSalesItem[]>,
        api.dashboardRestockAlerts(activeWarehouseId) as Promise<RestockAlertItem[]>,
      ]);
      setSummary(s);
      setCharts(c);
      setAlerts(a || []);
          ...
          <div style={{ display: "grid", gap: 6 }}>
            {zeroSales.slice(0, 20).map((item) => (
              ...
              >
                ...
                <span style={{ fontSize: 12, color: "#5a5f6e", flex: 1 }}>{item.ten_san_pham}</span>
              ...
              >
                ...
                <span
                  style={{
                    ...
                    padding: "2px 6px",
                    borderRadius: 3,
                    background: alert.alert_type === "ton_lau" ? "#fef3e2" : "#fde8e8",
                    color: alert.alert_type === "ton_lau" ? "#b07a2a" : "#b83b3b",
                  }}
                >
                  {alert.alert_type === "ton_lau" ? "Tồn lâu" : "Thiếu hàng"}
                </span>
                <span style={{ fontSize: 11, color: "#7a7f8e" }}>{alert.message}</span>
              </div>
            ))}
          </div>
        </div>
