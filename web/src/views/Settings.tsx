...
export function Settings() {
  ...
  const fetchHistory = useCallback(
    async (sku: string) => {
      ...
      try {
        const data = (await api.getThresholds(sku, activeWarehouseId)) as ThresholdEntry[];
        setHistory(data || []);
      } catch (err) {
  ...
  const handleDownloadTemplate = useCallback(async () => {
    try {
      ...
      alert(`Tải mẫu thất bại: ${err instanceof Error ? err.message : "Unknown"}`);
    }
  }, []);
  ...
  const handleUpload = useCallback(async () => {
    ...
    try {
      ...
      alert(`Upload thất bại: ${err instanceof Error ? err.message : "Unknown"}`);
    } finally {
      setUploading(false);
    }
  }, []);
  ...
  const handleRecalcAll = useCallback(async () => {
    ...
    try {
      ...
      alert(`Recalc thất bại: ${err instanceof Error ? err.message : "Unknown"}`);
      setRecalcLoading(false);
    }
  }, []);
    ...
    <div>
      ...
      >
        ...
        <p style={{ fontSize: 11, color: "#7a7f8e", marginBottom: 12 }}>
          Upload file .xlsx (15 cột): Mã vạch, Tên sản phẩm, BU, Mã cat, Mã nhóm hàng, Nhóm hàng,
          ĐVT, Quy cách, Đơn giá, VAT, Ngày cập nhật, Hoa hồng, Mã lô hàng, Ngày nhập, Số tồn. Số
          nhập/Số xuất được tính tự động từ đơn nhập/xuất. Giá NIV, Đơn giá nhập, Tiền tồn được tự
          động tính. Mỗi mã vạch có nhiều lô → nhập nhiều dòng. Ngày dùng dd/mm/yyyy, dd-mm-yyyy,
          hoặc dd-mm-yy. Mã lô hàng nếu để trống sẽ tự tạo LOT-&lt;mã vạch&gt;-&lt;yyyymmdd&gt;.
        </p>
          ...
          >
            ...
            >
              <div>
                ...
                {importJob.error && (
                  <span style={{ color: "#b83b3b", marginLeft: 8 }}>{importJob.error}</span>
                )}
              </div>
                        ...
                        >
                          {shown.map((err, i) => (
                            <div
                              ...
                              style={{
                                padding: "3px 0",
                                borderBottom: i < shown.length - 1 ? "1px solid #f2f3f5" : "none",
            ...
            {importBatch && importBatch.success_rows === 0 && importBatch.error_rows > 0 && (
              <div
                style={{
                  marginTop: 8,
                  padding: "8px 12px",
              ...
              >
                Không có dòng nào import thành công. Kiểm tra định dạng file và dữ liệu.
              </div>
            )}

            {/* Summary row */}
            {importBatch && (importJob.status === "completed" || importJob.status === "failed") && (
              <div style={{ marginTop: 6, fontSize: 11, color: "#7a7f8e" }}>
                Tổng: {importBatch.total_rows} | Thành công: {importBatch.success_rows} | Lỗi:{" "}
                {importBatch.error_rows}
              </div>
            )}
          </div>
        )}
      ...
      >
        ...
        {historyLoading ? (
          ...
          <p style={{ color: "#7a7f8e", fontSize: 12 }}>
            {maHang ? "Chưa có ngưỡng cho mã này." : "Nhập mã hàng để xem lịch sử."}
          </p>
        ) : (
          <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 12 }}>
            <thead>
              <tr style={{ borderBottom: "1px solid #e8eaed", textAlign: "left" }}>
                <th
                  style={{
                    padding: "6px 10px",
                    color: "#7a7f8e",
      ...
      >
        ...
        <p style={{ fontSize: 11, color: "#7a7f8e", marginBottom: 12 }}>
          Tính lại tiền tồn/nhập/xuất, LBBQ, số ngày tồn bán cho tất cả SKU. Dùng khi dữ liệu đơn
          giá thay đổi hoặc metrics bị lệch.
        </p>
        <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
          <button
      ...
      >
        ...
        <p style={{ fontSize: 11, color: "#7a7f8e", marginBottom: 12 }}>
          Xóa toàn bộ dữ liệu trong database (sản phẩm, tồn kho, nhập/xuất, lô hàng, đơn hàng,
          ngưỡng...). Hành động này không thể hoàn tác.
        </p>
        <button
          onClick={() => {
        ...
        >
          ...
          >
            ...
            <p style={{ fontSize: 12, color: "#3a3f4b", marginBottom: 16 }}>
              Hành động này sẽ <strong>xóa toàn bộ</strong> dữ liệu trong database. Không thể hoàn
              tác.
            </p>
            ...
            <input
              ...
              style={{
                ...
                width: "100%",
                marginBottom: 16,
                borderColor: resetConfirmText === "RESET ALL" ? "#3a7d4f" : "#d5d8de",
              }}
            />
            <div style={{ display: "flex", gap: 10, justifyContent: "flex-end" }}>
              ...
              <button
                ...
                style={{
                  padding: "7px 16px",
                  background: resetConfirmText === "RESET ALL" ? "#b83b3b" : "#e8eaed",
                  color: resetConfirmText === "RESET ALL" ? "#fff" : "#7a7f8e",
                  border: "none",
                  borderRadius: 5,
                  cursor:
                    resetConfirmText === "RESET ALL" && !resetting ? "pointer" : "not-allowed",
                  fontSize: 12,
                  fontWeight: 500,
                }}
              >
