...
// Returns the ma_hang of the moved card for downstream recalculation.
func (r *PostgresRepo) MoveKanbanInbound(ctx context.Context, id int64, toStage string, userID string) (maHang string, err error) {
	...
	return card.MaHang, tx.Commit(ctx)
}
...
// Returns the ma_hang of the moved card for downstream recalculation.
func (r *PostgresRepo) MoveKanbanOutbound(ctx context.Context, id int64, toStage string, userID string) (maHang string, negativeAlert bool, err error) {
	tx, err := r.Pool.Begin(ctx)
	if err != nil {
	...
	return card.MaHang, negativeAlert, tx.Commit(ctx)
}
