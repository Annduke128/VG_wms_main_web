...
import type { FilterItem, GridRequest, GridResponse, SortItem } from "../types/grid";
...
export function InventoryGrid({ onRowSelect, warehouseId }: InventoryGridProps) {
  ...
  const filterModel = useMemo(() => {
    ...
    if (buFilter) fm.ma_bu = { filterType: "text", type: "contains", filter: buFilter };
    if (nhomFilter)
      fm.ma_nhom_hang = {
        filterType: "text",
        type: "contains",
  ...
  useEffect(() => {
    api
      .inventoryFilterOptions(warehouseId)
  ...
  const fetchRange = useCallback(
    async (startRow: number, endRow: number) => {
      ...
      try {
        ...
        const resp = (await api.inventoryGrid(req)) as GridResponse<InventoryMain>;
    ...
    [sortModel, filterModel, warehouseId],
  );
  ...
  const onCellEdited = useCallback(
    ([col, row]: Item, newValue: { data: unknown }) => {
      ...
      api.updateInventory(item.ma_hang, { [field]: newValue.data }, warehouseId).catch(() => {
        setData((prev) => {
          const next = [...prev];
  ...
  const onColumnResize = useCallback((column: GridColumn, newSize: number) => {
    setCols((prev) => prev.map((c) => (c.id === column.id ? { ...c, width: newSize } : c)));
  }, []);
  ...
  const handleExport = useCallback(async () => {
    const selectedCols = ALL_EXPORT_COLUMNS.filter((c) => exportColumns.has(c.id)).map((c) => c.id);
    ...
    try {
      ...
      alert(`Xuất báo cáo thất bại: ${err instanceof Error ? err.message : "Unknown"}`);
    } finally {
      setExporting(false);
    }
  }, [exportColumns, selectedMaHangs, filterModel, warehouseId]);
  ...
  return (
    ...
    >
      ...
      >
        ...
        {loading && <span style={{ color: "#7a7f8e", fontSize: 12 }}>Loading...</span>}
          ...
          >
            ...
            >
              <div style={{ display: "flex", gap: 8 }}>
                <button
                  type="button"
                  onClick={() => setExportColumns(new Set(ALL_EXPORT_COLUMNS.map((c) => c.id)))}
                  style={{
                    padding: "5px 10px",
                    fontSize: 11,
                    background: "#fff",
              ...
              <div style={{ display: "flex", gap: 8 }}>
                ...
                <button
                  ...
                  style={{
                    ...
                    border: "none",
                    borderRadius: 5,
                    cursor: exporting || exportColumns.size === 0 ? "not-allowed" : "pointer",
                    fontSize: 12,
                    fontWeight: 500,
                  }}
                >
